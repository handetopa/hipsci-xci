get_snpwise_ase <- function(ase_fileName,biomart_ase,gene=NULL,chr=NULL,write_file=TRUE) {
  
  library(rdist)
  load("/Users/topah/Desktop/gene_info/ref_hs37d5.RData")
  #load("biomart_GRCh37_75_wPositions.Rdata")
  biomart_ase=ref
  source("~/Projects/HipSci_GTEx/scripts/position_scripts.R")
  source("/Users/topah/Projects/puhti_ase/run_scripts/scripts/extract_text_before_first_dot.R")
  if (!file.exists(ase_fileName)) {
    print(paste("No ASE file found for ",ase_fileName,sep=""))
  } else {
    library(dplyr)
    ...
    if (!is.null(gene)) {
      j=which(biomart_ase$ensembl_gene_id==gene | biomart_ase$external_gene_id==gene)
    } else if (!is.null(chr)) {
      j=which(biomart_ase$chromosome_name==chr)
    } else {
      j=(1:dim(biomart_ase)[1])
    }
    gene_name=biomart_ase$external_gene_id[j]
    gene_id=biomart_ase$ensembl_gene_id[j]
    chromosome=biomart_ase$chromosome_name[j]
    start_position=biomart_ase$start_position[j]
    end_position=biomart_ase$end_position[j]
    gene_length=biomart_ase$end_position[j]-biomart_ase$start_position[j]+1
    gene_ase_info=apply(as.matrix(j), 1, function(x) get_ase_ratio2(get_variant_info(ase_info,chr=biomart_ase$chromosome_name[x],select_position_vector=unlist(biomart_ase[x,4:5]),min_total_count=20,min_read_length=100)))
    ...
    if (!write_file) {
      ...
      #write.table(df,file=paste(extract_text_before_first_dot(ase_fileName),"_min20_snpwiseASE_withGenePval_new.txt",sep=""), quote=FALSE, row.names=FALSE, sep="\t")
      write.table(df,file=paste(gsub("\\-.*", "", ase_fileName1),"_min20_snpwiseASE_withGenePval_new2.txt",sep=""), quote=FALSE, row.names=FALSE, sep="\t")
    }
  }
}

...
boxplot_esc_ratio_in_female_groups_per_xci_group <- function(human_xci=NULL) {
  ...
  ggsave("figures/boxplot_esc_ratio_in_female_groups_per_xci_group_with_wrong_pvals.pdf", p.xci.xist, width=24,height=19,units="cm",limitsize = FALSE)
  ...
  print(paste("Paired one-sided Wilcoxon p-value for inactive genes: ",wilcox.test(BB,AA,paired=TRUE)$p.value,sep=""))

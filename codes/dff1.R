...
get_dff1 <- function() {
  ...
  ddf=data.frame(num_escape_all=rowSums(sig,na.rm=TRUE),
               ...
               num_escape_Group1=rowSums(sig[,clusts=="high"],na.rm=TRUE),
               num_observed_Group1=rowSums(!is.na(sig[,clusts=="high"])),
               num_lines_Group1=sum(clusts=="high"),
               num_escape_Group2=rowSums(sig[,clusts=="highescape"],na.rm=TRUE),
               num_observed_Group2=rowSums(!is.na(sig[,clusts=="highescape"])),
               num_lines_Group2=sum(clusts=="highescape"),
               num_escape_Group3=rowSums(sig[,clusts=="low"],na.rm=TRUE),
               num_observed_Group3=rowSums(!is.na(sig[,clusts=="low"])),
               num_lines_Group3=sum(clusts=="low"))
  ...
  dff1$num_escape_genes=colSums(sig,na.rm=TRUE)
  dff1$num_observed_genes=colSums(!is.na(sig))
  dff1$fraction_of_genes_escaping=colSums(sig,na.rm=TRUE)/colSums(!is.na(sig))
  dff1$XIST_cluster[which(dff1$XIST_cluster=="low")]="Group3"
  dff1$XIST_cluster[which(dff1$XIST_cluster=="highescape")]="Group2"
  ...
  median(dff1$median_ASE[which(dff1$XIST_group=="Low-XIST female")])
  # 0.0499343
  median(dff1$median_ASE[which(dff1$XIST_group=="High-XIST female")])
  # 0.01211148
  median(dff1$mean_ASE[which(dff1$XIST_group=="High-XIST female")])
  # 0.1014025
  median(dff1$mean_ASE[which(dff1$XIST_group=="Low-XIST female")])
  # 0.1790307
  ...
  plot(ase_ratios[xist_ind,],p_est)

  dff1=data.frame(cell_line=D$lines,XIST_logCPM=D$xist_cpm_log,XIST_group=D$xist_group,XIST_cluster=clusts)
  dff1$XIST_ASE=ase_ratios[xist_ind,]
  dff1$XIST_ASE_CI_lower=low_xist
  dff1$XIST_ASE_CI_upper=up_xist
  dff1$XIST_is_monoallelic_binom.pval=pval.xist
  dff1$mean_ASE=apply(ase_ratios,2,mean,na.rm=TRUE)
  dff1$median_ASE=apply(ase_ratios,2,median,na.rm=TRUE)
  dff1$num_escape_genes=colSums(sig,na.rm=TRUE)
  dff1$num_observed_genes=colSums(!is.na(sig))
  dff1$fraction_of_genes_escaping=colSums(sig,na.rm=TRUE)/colSums(!is.na(sig))
  dff1$num_escape_genes_min10lines_filtered=colSums(sig[ttt,],na.rm=TRUE)
  dff1$num_observed_genes_min10lines_filtered=colSums(!is.na(sig[ttt,]))
  dff1$fraction_of_genes_escaping_min10lines_filtered=colSums(sig[ttt,],na.rm=TRUE)/colSums(!is.na(sig[ttt,]))
  dff1$XIST_cluster[which(dff1$XIST_cluster=="low")]="Group3"
  dff1$XIST_cluster[which(dff1$XIST_cluster=="highescape")]="Group2"
  dff1$XIST_cluster[which(dff1$XIST_cluster=="high")]="Group1"
